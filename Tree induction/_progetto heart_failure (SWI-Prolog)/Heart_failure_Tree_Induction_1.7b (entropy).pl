...
% di un albero decisionale e di apprenderlo al termine
...
induce_albero( Attributi, Esempi, t(Attributo,SottoAlberi) ) :-   % caso in cui gli esempi sono di classi differenti: viene creato un nodo al quale sono associati un attributo e dei sottoalberi che si genereerà da questo
...
induce_albero( _, Esempi, l(C):P) :-		% caso in cui non vi sono più attributi utili alla classificazione: si forma una foglia assegnando una calsse C con probabilità P
...
somma(_,_,[],Pbase,Pbase).						   % tappo della ricorsione caso in cui non ci sono più valori
somma( Esempi, Att, [Val|Valori], SommaParziale, Somma) :-
	...
	NVal > 0, !,                                                       % se c'è almeno un esempio soddisfatto
	findall(P,                                                         %
                (bagof(1,                                                  % aggiunge un 1 alla lista L per ogni esempio soddisfatto
                       member(y,EsempiSoddisfatti),                        % della classe y
                       L),                                                 %
                 length(L,NVC),                                            % calcola il numero NVC di esempi soddisfatti con classe y
                 P is NVC/NVal),                                           % calcola la probabilità P di avere una classe y tra gli esempi soddisfatti
                Pp),                                                       % Pp è una lista che contiene la probabilità P !!!!!!!!!NON HA SENSO!!!!!!!!
        entropia(Pp,E),                                                    % restituisce in E il contributo dell'entropia del valore Val passando come argomento Pp
        NuovaSommaParziale is SommaParziale + E*NVal/N,                    % moltiplica E per la probabilità di avere quel valore e lo somma agli altri contributi in NuovaSommaParziale
...
% entropia(+Probabilità,-Entropia) restituisce l'Entropia data la
% probabilità P
entropia([],0).                   % caso in cui la probabilità è 0 l'entropia è 0
entropia([1],0).                  % caso in cui la probabilità è 1 l'entropia è 0
entropia([P],E):-		  % caso in cui la probabilità è diversa da 0 e 1
        log(P,L1),
        log((1-P),L2),
        E is -1*(P*L1+(1-P)*L2).
...
% di Att
induce_alberi(_,[],_,_,[]).                                                %tappo della ricorsione: caso in cui la lista dei valori è vuota, non induce nessun sottoalbero
induce_alberi(Att,[Val1|Valori],AttRimasti,Esempi,[Val1:Alb1|Alberi])  :-
	esempi_AttVal(Att=Val1,Esempi,SottoinsiemeEsempi),                 % crea la lista SottoinsiemeEsempi di esempi presi dalla lista Esempi che soddisfa la coppia Att = Val1
...
% Sottoinsieme che contiene gli esempi che soddisfano la coppia Att =
% Val
...
% in Oggetto non è presenta la coppia Att=Val
soddisfa(Oggetto,Congiunzione)  :-
	\+ (member(Att=Val,Congiunzione), % unifica Att e Val con la coppia attributo valore in Congiunzione
...
% Rimanenti
rimuovi(T,[T|C],C) :- !.    %tappo della ricorsione: caso in cui l'attributo da rimuovere è in testa
rimuovi(A,[T|C],[T|C1]) :-  %caso in cui l'attributo non è in testa
